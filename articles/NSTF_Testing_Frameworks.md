...
(i.e. `RUnit‑based`, `BiocGenerics`, `data.table`, `tinytest`, and
`custom test layouts`), it runs non-standard testing workflows depending
...
- Calls
  ...
  to load the package under test.

- Clears `R_TESTS` to avoid R CMD check harness interference when
  running RUnit programmatically.

- Identifies problem/skip-able tests (which should use
  [`RUnit::runTestFile()`](https://rdrr.io/pkg/RUnit/man/runit.html) +
  [`RUnit::getErrors()`](https://rdrr.io/pkg/RUnit/man/textProtocol.html)
  per file).

- Builds a list of test files **under `inst/unitTests`** excluding any
  that should be skipped.

- Exposes namespace objects + datasets to a dedicated environment and
  runs coverage with
  [`covr::environment_coverage()`](http://covr.r-lib.org/reference/environment_coverage.md).

- If `BiocGeneric workflow` is successful, then a unit test data
...
The `NSTF unit test data coverage objects` are not able to give details
about test block start and end lines and the expectation types.
