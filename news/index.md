...
- increase test coverage for `test-map_tests_stf`,
  `test-unittest_methods`
...
- this version fixed issues found in the CRAN check including:
  ...
  - run_covr_modes - sets up the coverage env to avoid writing by
    default in the user’s home filespace
  - run_covr_skip_stf.R - uses the coverage env to avoid writing in the
    user’s home filespace
  - run_covr_skip_nstf.R - uses the coverage env to avoid writing in the
    user’s home filespace
...
- Run test coverage for:
  ...
  - standard testing frameworks (STF) - tests located in `testthat`,
    `testit`,
