...
test_that("generate_test_report returns message when directory does not exist", {
  # Mock the fs::dir_exists function to return FALSE
  mockery::stub(generate_test_report, "fs::dir_exists", FALSE)
  
  # Define the output directory
  output_dir <- "mock_output_dir"
  
  # Capture the message output
  expect_message(output_file <- generate_test_report(toy_assessment_results, output_dir), "The output directory does not exist.")
...
stub_system_file <- function(...) {
  args <- list(...)
  # Expect args like: "report_templates", "<filename>", package = "test.assessr"
  filename <- if (length(args) >= 2) args[[2]] else "unknown"
...
test_that("standard testing framework (nstf_none) uses STF template and renders", {
  ...
  # NSTF none
  mockery::stub(generate_test_report, "check_elements_simple",
  ...
  mockery::stub(generate_test_report, "render", mock_render)
  
  expect_message(
    res <- generate_test_report(tr, output_dir = outdir),
    "Rendering report for pkg"
  )
  
  expected_outfile <- file.path(outdir, paste0("test_report_", tr$pkg_name, "_", tr$pkg_ver, ".html"))
  expect_equal(res, expected_outfile)
  
  mockery::expect_called(mock_render, 1)
  args <- mockery::mock_args(mock_render)[[1]]
  expect_true(grepl("test_report_template_stf\\.Rmd$", args$input))
  expect_equal(args$output_file, expected_outfile)
...
test_that("non-standard testing framework (nstf present) uses NSTF template and processes data frames", {
  ...
  # Convert + path shortener
  mockery::stub(generate_test_report, "convert_vectors_to_df",
       function(test_results, name) list(data.frame(
  ...
  mockery::expect_called(mock_render, 1)
  args <- mockery::mock_args(mock_render)[[1]]
  expect_true(grepl("test_report_template_nstf\\.Rmd$", args$input))
  expect_equal(args$output_file, expected_outfile)
  
  # Environment passed to render should contain processed NSTF data
  env <- args$envir
  ...
  expect_equal(env$tests_passing_df$file, c("file_one.R", "file_two.R"))
})


test_that("uses getwd() and emits message when output_dir is NULL", {
  tr <- make_test_results_stf()
  ...
  expect_message(
    res <- generate_test_report(tr, output_dir = NULL),
  ...
  expect_true({
    ...
    res_norm <- tolower(sub("^//\\?/", "", norm(res)))
    wd_norm  <- tolower(sub("^//\\?/", "", norm(wd)))
    startsWith(res_norm, paste0(wd_norm, "/"))
  })
  
  expect_true(grepl(paste0("test_report_", tr$pkg_name, "_", tr$pkg_ver, "\\.html$"), res))
  mockery::expect_called(mock_render, 1)
})
