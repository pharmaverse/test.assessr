test_that("run_covr_skip_stf filters test results and generates skip map", {
  ...
  # Stub create_coverage_skip_stf 
  mockery::stub(run_covr_skip_stf, "create_coverage_skip_stf", 
                function(pkg_source_path, pkg, 
                         test_path, 
                         test_skip, 
                         test_map, 
                         preserve_vars) {
    structure(list(package = "mockpkg", coverage = 85.5), class = "coverage")
  })
  ...
  mockery::stub(run_covr_skip_stf, "create_fail_reporter", function(...) {
    list() # Return an empty list or mock fail data
  })
  mockery::stub(run_covr_skip_stf, "create_test_reporter", function(test_path, pkg) {
    # Return a mock test results list similar to what fix_test_context would produce
    list(
  ...
  mockery::stub(run_covr_skip_stf, "cleanup_and_return_null", function(env, preserve_vars) NULL)
...
test_that("fallback problem_tests is created when no failed/skipped/error tests", {
  ...
  mockery::stub(run_covr_skip_stf, "create_test_reporter", function(test_path, pkg) {
    list(list(file = "test-a.R", context = "script1"))
  })
  
  mockery::stub(run_covr_skip_stf, "create_fail_reporter", function(test_path, pkg) {
    list() # Return empty list to bypass testthat::test_dir()
  })
  ...
  # Stub get_pkg_name to return a fake package name
  mockery::stub(run_covr_skip_stf, "get_pkg_name", function(...) "mockpkg")
  
  # Stub get_stf_test_path to return a fake test directory
  mockery::stub(run_covr_skip_stf, "get_stf_test_path", function(...) "tests")
  
  # Stub create_test_reporter so it doesn't call testthat::test_dir()
  mockery::stub(run_covr_skip_stf, "create_test_reporter", function(test_path, pkg) {
    # Return a mock test results list
    list(
      list(file = "test-a.R", context = "script1"),
      list(file = "test-b.R", context = "script2")
    )
  ...
  expect_true(is.null(result$res_cov$failed) || result$res_cov$failed == 0)
})
...
test_that("create_coverage_skip_stf runs coverage with datasets", {
  ...
  # Stub load_package_datasets to simulate datasets being loaded
  mockery::stub(create_coverage_skip_stf, "load_package_datasets", function(pkg, env) {
    list("data1", "data2")
  })
  
  # Stub find_project_root to return a fake root directory
  mockery::stub(create_coverage_skip_stf, "find_project_root", function() {
    "fake/root"
  })
  
  # Stub safe_unload_package to do nothing
  mockery::stub(create_coverage_skip_stf, "safe_unload_package", function(pkg) NULL)
  
  # Stub readLines to simulate file content
  mockery::stub(create_coverage_skip_stf, "readLines", function(file, warn) {
    c("#test_that('example', {", "#expect_true(TRUE)", "})")
  })
  
  # Stub cat to suppress file writing
  mockery::stub(create_coverage_skip_stf, "cat", function(..., file, sep) NULL)
  
  # Stub covr::environment_coverage to return a mock coverage object
  mock_cov <- structure(list(), class = "coverage")
  mockery::stub(create_coverage_skip_stf, "covr::environment_coverage", function(env, test_files) {
    mock_cov
  })
  
  # Stub run the code without changing directory
  mockery::stub(create_coverage_skip_stf, "withr::with_dir", function(new, code) {
    force(code)  
  })
  
  
  # Run the function
  result <- create_coverage_skip_stf(pkg, test_path, test_skip, test_map)
  
  # Since the function returns invisibly, we check side effects via coverage call
  expect_s3_class(mock_cov, "coverage")
})
...
test_that("create_covr_list_no_skip returns structured coverage list", {
  ...
  mockery::stub(
    ...
    function(pkg_source_path, pkg, cov_env, preserve_vars = "old") TRUE
  )
  ...
  mockery::stub(
    ...
    "data",
    function(package, ...) {
      # Return shape compatible with utils::data(...), i.e. list(results = <matrix|df>)
...
test_that("create_covr_list_no_skip falls back to package_coverage when no datasets used", {
  ...
  mockery::stub(create_covr_list_no_skip, "data", function(package) {
...
test_that("create_covr_list_no_skip handles NULL coverage", {
  # Mock inputs
  test_map <- data.frame(file = "test-file.R")
  test_results_long <- list(
    list(results = list(
      structure(list(), class = "expectation_success"),
      structure(list(), class = "expectation_failure"),
      structure(list(), class = "expectation_skip")
    ))
  )
  pkg_source_path <- tempdir()
  pkg <- "mockpkg"
  
  # Stub functions to simulate NULL coverage
  mockery::stub(create_covr_list_no_skip, "safe_unload_package", NULL)
  mockery::stub(create_covr_list_no_skip, "load_package_into_env", NULL)
  mockery::stub(create_covr_list_no_skip, "data", list(results = data.frame(Item = character())))
  mockery::stub(create_covr_list_no_skip, "covr::package_coverage", NULL)
  mockery::stub(create_covr_list_no_skip, "cleanup_and_return_null", NULL)
  ...
  expect_equal(result$res_cov$coverage$filecoverage,
...
test_that("create_coverage_skip_stf reports no helpers when none exist", {
  ...
  # Mock setup_covr_env to return empty helpers
  mockery::stub(create_coverage_skip_stf, "setup_covr_env", function(...) {
    list(
      cov_env = new.env(),
  ...
  mockery::stub(create_coverage_skip_stf, "covr::environment_coverage", function(...) structure(list(), class = "coverage"))
  ...
  expect_true(any(grepl("No helper files were loaded.", messages)))
})
...
test_that("create_coverage_skip_stf reports loaded helpers when helpers exist", {
  ...
  # Stub setup_covr_env to return helpers
  mockery::stub(create_coverage_skip_stf, "setup_covr_env", function(...) {
    list(
      cov_env = new.env(),
      datasets_loaded = character(),
      root_dir = pkg_dir,
      initial_state = character(),
      helpers_loaded = c("helper1.R", "helper2.R")
    )
  })
  ...
  expect_true(any(grepl("Helper files loaded:", messages)))
  expect_true(any(grepl("helper1.R", messages)))
  expect_true(any(grepl("helper2.R", messages)))
})
